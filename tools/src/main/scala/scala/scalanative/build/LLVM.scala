...
private[scalanative] object LLVM {
  ...
  def compile(config: Config, llPaths: Seq[Path]): Seq[Path] = {
    ...
    val flags = clangOptions(config, config.compileOptions ++ Seq(optimizationOpt))

    llPaths.par
      .map { ll =>
        val apppath = ll.abs
        val outpath = apppath + oExt
        val compile = Seq(config.clang.abs) ++ flags ++ Seq("-c", apppath, "-o", outpath)
  ...
  def link(config: Config,
    ...
    val flags       = clangOptions(config, linkopts ++ Seq("-rdynamic"))
    val paths       = objectFiles.map(_.abs)
    val compile     = Seq(config.clangPP.abs) ++ flags ++ paths ++ Seq("-o", outpath.abs)
