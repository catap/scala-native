...
import scala.scalanative.unsafe.{CFuncPtr1, CInt, CSize, Ptr, stackalloc}
...
class Thread private (
  ...
  private def this(group: ThreadGroup,
           target: Runnable,
           name: String,
           stacksize: CSize) = {
  ...
  def this(group: ThreadGroup,
    ...
    this(group, target, name, stackSize.toUInt)

  def this() =
    this(null, null, Thread.THREAD, NativeThread.THREAD_DEFAULT_STACK_SIZE)

  def this(target: Runnable) =
    this(null, target, Thread.THREAD, NativeThread.THREAD_DEFAULT_STACK_SIZE)

  def this(target: Runnable, name: String) =
    this(null, target, name, NativeThread.THREAD_DEFAULT_STACK_SIZE)

  def this(name: String) =
    this(null, null, name, NativeThread.THREAD_DEFAULT_STACK_SIZE)

  def this(group: ThreadGroup, target: Runnable) =
    this(group, target, Thread.THREAD, NativeThread.THREAD_DEFAULT_STACK_SIZE)

  def this(group: ThreadGroup, target: Runnable, name: String) =
    this(group, target, name, NativeThread.THREAD_DEFAULT_STACK_SIZE)

  def this(group: ThreadGroup, name: String) =
    this(group, null, name, NativeThread.THREAD_DEFAULT_STACK_SIZE)
...
object Thread extends scala.scalanative.runtime.ThreadModuleBase {
  ...
  private val callRunRoutine: CFuncPtr1[Ptr[scala.Byte], Ptr[scala.Byte]] = {
    (p: Ptr[scala.Byte]) =>
  ...
  private def toNativePriority(priority: Int) = {
    val range =
      NativeThread.THREAD_MAX_PRIORITY - NativeThread.THREAD_MIN_PRIORITY
    if (range == 0) {
  ...
  private def findSelf(id: pthread_t, group: ThreadGroup): Option[Thread] = {
    val threads: scala.collection.immutable.List[Thread]     = group._threads
    val groups: scala.collection.immutable.List[ThreadGroup] = group._groups
  ...
  private val currentThreadStackTracePtr: CFuncPtr1[CInt, Unit] = { (_: CInt) =>
    try {
  ...
  private val currentThreadStackTraceSignal = signal.SIGUSR1
  signal.signal(currentThreadStackTraceSignal, currentThreadStackTracePtr)

  private val currentThreadSuspendPtr: CFuncPtr1[CInt, Unit] = { (_: CInt) =>
    try {
  ...
  private val suspendSignal = signal.SIGUSR2
