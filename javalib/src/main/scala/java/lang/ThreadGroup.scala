...
class ThreadGroup private[lang] (
  ...
  def threads: util.List[Thread] = {
    val threadsCopy = _threads
    val utilThreads = new util.ArrayList[Thread](threadsCopy.size)
    for (thread: Thread <- threadsCopy) {
      utilThreads.add(thread)
    }
    utilThreads
  }

  def groups: util.List[ThreadGroup] = {
    val groupsCopy = _groups
    val utilGroups = new util.ArrayList[ThreadGroup](groupsCopy.size)
    for (threadGroup: ThreadGroup <- groupsCopy) {
      utilGroups.add(threadGroup)
    }
    utilGroups
  }
  ...
  def nonDaemonThreadExists: scala.Boolean = {
    lock.safeSynchronized {
      _threads.exists(t => !t.isDaemon) || _groups.exists(
        _.nonDaemonThreadExists)
    }
  }
