...
final class PosixMonitor private[runtime] (shadow: Boolean) {
  ...
  def _wait(millis: scala.Long, nanos: Int): Unit = {
    ...
    val deadLineSeconds =
      curSeconds + millis / 1000 + overflownNanos / 1000000000
...
object PosixMonitor {
  private val mutexAttrPtr: Ptr[pthread_mutexattr_t] =
    Boxes.boxToPtr(malloc(pthread_mutexattr_t_size))
  pthread_mutexattr_init(mutexAttrPtr)
  pthread_mutexattr_settype(mutexAttrPtr, PTHREAD_MUTEX_RECURSIVE)

  private val condAttrPtr: Ptr[pthread_condattr_t] =
    Boxes.boxToPtr(malloc(pthread_condattr_t_size))
  pthread_condattr_init(condAttrPtr)
  pthread_condattr_setpshared(condAttrPtr, PTHREAD_PROCESS_SHARED)

  private[runtime] val monitorCreationMutexPtr: Ptr[pthread_mutex_t] =
    Boxes.boxToPtr(malloc(pthread_mutex_t_size))
  ...
  private def unsafeCreate(x: Object) =
    new PosixMonitor(x.isInstanceOf[ShadowLock])
}
