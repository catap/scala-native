...
abstract class CountedCompleter[T] protected extends ForkJoinTask[T] {
  ...
  def onExceptionalCompletion(ex: Throwable,
                              caller: CountedCompleter[_]): Boolean = true
...
class ForkJoinPool(parallelism: Int,
                   val factory: ForkJoinPool.ForkJoinWorkerThreadFactory,
                   val ueh: Thread.UncaughtExceptionHandler,
                   asyncMode: Boolean)
  ...
  def this(p: Int) =
    this(p, ForkJoinPool.defaultForkJoinWorkerThreadFactory, null, false)
  ...
  /*no longer volatile*/
  var config: Int = parallelism | (if (asyncMode) (FIFO_QUEUE << 16)
                                   else 0) // mode and parallelism level
  var workQueues: Array[WorkQueue] = null // main registry
  final var workerNamePrefix
      : String = sb.toString // to create worker name string
  ...
  private def tryAddWorker(): Unit = {
    ...
    while (u < 0 && (u & SHORT_SIGN) != 0 && c.toInt == 0 && !break) {
      val nc: Long = (((u + UTC_UNIT) & UTC_MASK) | ((u + UAC_UNIT) & UAC_MASK))
  ...
  def registerWorker(wt: ForkJoinWorkerThread): WorkQueue = {
    ...
    do { s = indexSeed; s1 = s + indexSeed.load() } while (!indexSeed
      .compareAndSwapStrong(s, s1)) // skip 0

    val w: WorkQueue = new WorkQueue(this, wt, config >>> 16, s)
    if (((ps & PL_LOCK) != 0) || !plock.compareAndSwapStrong(ps, ps + PL_LOCK))
      ps = acquirePlock
    ...
    } finally if (!plock.compareAndSwapStrong(ps, nps)) releasePlock(nps)
  ...
  def deregisterWorker(wt: ForkJoinWorkerThread, ex: Throwable): Unit = {
    ...
    if (wt != null && w != null) {
      ...
      do { sc = stealCount } while (!stealCount.compareAndSwapStrong(sc,
                                                                     sc + ns))
      ps = plock
      if ((ps & PL_LOCK) != 0 || !plock.compareAndSwapStrong(
            ps, { ps = ps + PL_LOCK; ps }))
        ps = acquirePlock
      val nps: Int = (ps & SHUTDOWN) | ((ps + PL_LOCK) & ~SHUTDOWN)
      try {
    ...
    do { c = ctl } while (!ctl.compareAndSwapStrong(
      c,
      ...
      while (u < 0 && e >= 0 && !break) {
        if (e > 0) { // activate or create replacement
          if (ws == null || i >= ws.length || v == null)
            break = true
          val nc: Long =
            (v.nextWait & E_MASK).toLong | ((u + UAC_UNIT) << 32).toLong
  ...
  def externalPush(task: ForkJoinTask[_]): Unit = {
    ...
    if (z != null && plock > 0 && ws != null && m >= 0 && q != null && q.qlock
          .compareAndSwapStrong(0, 1)) { // lock
      val b: Int  = q.base
      val s: Int  = q.top
      val n: Int  = s + 1 - b
      val an: Int = a.length
  ...
  private def fullExternalPush(task: ForkJoinTask[_]): Unit = {
    ...
    while (true) {
      ...
      if (z == null) {
        ...
        n = (n + 1) << 1
        ws = workQueues
        val nws: Array[WorkQueue] =
          if (ws == null || ws.length == 0) new Array[WorkQueue](n) else null
        val ps1 = ps
        ps = ps + PL_LOCK
    ...
    while (u < 0 && !break) {
      ...
      if (e > 0) {
        ...
        if (ws != null && ws.length > i && w != null && w.eventCount == (e | INT_SIGN)) {
          val nc: Long =
            (w.nextWait & E_MASK).toLong | ((u + UAC_UNIT) << 32).toLong
          if (ctl.compareAndSwapStrong(c, nc)) {
            w.hint = hint
          ...
          if ((h >= 0 || { h = w.hint; h } >= 0) && q != null) { // signal others before retry
            ...
            while (!break) {
              ...
              if (s <= n1 && n <= 0 || u >= 0 || e <= 0 || m < i || v == null)
                break = true
              val nc: Long =
                (v.nextWait & E_MASK).toLong | (u + UAC_UNIT).toLong << 32
              if (v.eventCount != (e | INT_SIGN) || !ctl
                    .compareAndSwapStrong(c, nc) && !break)
                break = true
              if (!break) {
                v.hint = h
                v.eventCount = (e + E_SEQ) & E_MASK
  ...
  private def idleAwaitWork(w: WorkQueue,
                            currentCtl: Long,
                            prevCtl: Long): Unit = {
    if (w != null && w.eventCount < 0 && !tryTerminate(false, false) && prevCtl.toInt != 0 &&
      ...
      val parkTime: Long =
        if (dc < 0) FAST_IDLE_TIMEOUT else (dc + 1) * IDLE_TIMEOUT
      ...
      while (ctl == currentCtl && !break) {
        ...
        if (ctl != currentCtl)
          break = true
        if (deadline - System.nanoTime() <= 0L && ctl
              .compareAndSwapStrong(currentCtl, prevCtl) && !break) {
      ...
      while (j >= 0 && !breakOuter) {
        ...
        while (!breakInner) { // limit to at most m signals
          ...
          if (u >= 0 || e <= 0 || m < i || w == null)
            breakOuter = true
          val nc: Long =
            (w.nextWait & E_MASK).toLong | (u + UAC_UNIT).toLong << 32
        ...
        while (true) {
          ...
          while (!break && continueRestart && !breakRestart) { // help stealer or descend to its stealer
            ...
            if (b < 0 && a != null && continueRestart) {
              ...
              if (continueRestart && t != null && v.base == b && a(i)
                    .compareAndSwapStrong(t,
  ...
  def awaitJoin(joiner: WorkQueue, task: ForkJoinTask[_]): Int = {
    ...
    if (joiner != null && task != null && s >= 0) {
      ...
      do {
        s = task.status
      } while (s >= 0 && !joiner.isEmpty && joiner.tryRemoveAndExec(
        task
      )) // process local tasks
      s = task.status
      if (s >= 0 && s >= 0) {
  ...
  def helpQuiescePool(w: WorkQueue): Unit = {
    ...
    while (true) {
      ...
      if (q != null) {
        ...
        c = ctl
        if ((c >> AC_SHIFT).toInt + (config & SMASK) == 0 && ctl
              .compareAndSwapStrong(c, c + AC_UNIT))
          return
      }
    }
  }
        ...
        while (pass < 3) {
          ...
          if (ws != null) {
            ...
            while ((e & E_MASK) != 0 && i < n && i >= 0 && w != null) {
              val nc: Long =
                (w.nextWait & E_MASK).toLong | ((cc + AC_UNIT) & AC_MASK) |
                  (cc & (TC_MASK | STOP_BIT))
              if (w.eventCount == (e | INT_SIGN) && ctl
                    .compareAndSwapStrong(cc, nc)) {
                w.eventCount = (e + E_SEQ) & E_MASK
                w.qlock.store(-1)
                p = w.parker
                if (p != null) {
  ...
  private def externalHelpComplete(q: WorkQueue,
                                   root: ForkJoinTask[_]): Unit = {
    var a: Array[CAtomicRef[ForkJoinTask[_]]] = null
    var m: Int                                = 0
  ...
  override def invokeAll[T](
    ...
    val futures: util.ArrayList[Future[T]] =
  ...
  def awaitQuiescence(timeout: Long, unit: TimeUnit): Boolean = {
    ...
    if (thread.isInstanceOf[ForkJoinWorkerThread] && {
          wt = thread.asInstanceOf[ForkJoinWorkerThread]; wt
        }.pool == this) {
    ...
    while (!isQuiescent && { ws = workQueues; ws } != null && {
             m = ws.length - 1; m
           } >= 0) {
      if (!found) {
...
object ForkJoinPool {
  ...
  final class WorkQueue {
    ...
    var currentSteal
        : ForkJoinTask[_] = null // current non-local task being executed

    //volatile
    var pad10, pad11, pad12, pad13, pad14, pad15, pad16, pad17: Object = _
    var pad18, pad19, pad1a, pad1b, pad1c, pad1d: Object               = _

    def this(pool: ForkJoinPool,
    ...
    def growArray: Array[CAtomicRef[ForkJoinTask[_]]] = {
      ...
      if (oldA != null && oldMask >= 0 && t - b > 0) {
        ...
        do {
          ...
          x = oldA(oldj)
          if (x != null && oldA(oldj)
                .compareAndSwapStrong(x, null.asInstanceOf[ForkJoinTask[_]]))
            a(j).store(x)
          b = b + 1
        } while (b != t)
      }
      ...
      if (a != null && m >= 0) {
        ...
        while (s >= 0 && !break) {
          ...
          if (t == null)
            break = true
          if (a(j)
                .compareAndSwapStrong(t, null.asInstanceOf[ForkJoinTask[_]]) && !break) {
            top = s
            t
          }
        }
            ...
            if (t == task)
              if (s + 1 == top) { // pop
                if (!a(j).compareAndSwapStrong(
                  ...
                  removed =
                    a(j).compareAndSwapStrong(task, new ForkJoinPool.EmptyTask)
                }
                break = true
                ...
                if (a(j).compareAndSwapStrong(
                      t,
                      null.asInstanceOf[ForkJoinTask[_]])) top = s
                break = true
              }
            n -= 1
            if (n == 0 && !break) {
      ...
      while (b - top < 0 && a != null && !breakOuter) {
        ...
        if (o == null || !o.isInstanceOf[CountedCompleter[_]]) {
          ...
          if (!breakOuter) {
            ...
            while (!breakInner) {
              if (r == root) {
                if ((base == b) && a(j).compareAndSwapStrong(
                      t,
                      null.asInstanceOf[ForkJoinTask[_]])) {
                  base = b + 1
                  t.doExec
  ...
  private final val modifyThreadPermission: RuntimePermission =
    new RuntimePermission("modifyThread")

  final val par: Int = {
    var pp = Integer.parseInt(
      System.getProperty(
  ...
  def tryExternalUnpush(t: ForkJoinTask[_]): Boolean = {
    ...
    if (t != null && { z = submitters.get(); z } != null && { p = common; p } != null && {
          ws = p.workQueues; ws
        } != null && { m = ws.length - 1; m } >= 0 && {
          q = ws(m & z.seed & SQMASK); q
        } != null && { s = q.top; s } != q.base && { a = q.array; a } != null) {
