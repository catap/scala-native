...
class AtomicSuite {
  ...
  @Test def `Atomic Reference`(): Unit = {
    ...
    assertFalse(a.compareAndSet(null, thing2))
    assert(a.compareAndSet(thing1, thing2))
    assertEquals(a.get(), thing2)
  }

  @Test def `Atomic Integer`(): Unit = {
    ...
    assertFalse(a.compareAndSet(0, 2))
  ...
  @Test def `Atomic Integer is atomic`(): Unit = {
    val numThreads = 2
  ...
  @Test def `Atomic Long`(): Unit = {
    ...
    assertFalse(a.compareAndSet(0L, 2L))
  ...
  @Test def `Atomic Long is atomic`(): Unit = {
    val numThreads = 2
    testWithMinRepetitions() { n: Int =>
      var number = 0L
  ...
  @Test def `Atomic Reference Array`(): Unit = {
    ...
    // the wrong indexes
    assertFalse(a.compareAndSet(0, thing2, thing3))
    assertFalse(a.compareAndSet(1, null, thing1))
    assertFalse(a.compareAndSet(2, thing1, thing2))
    ...
    assertThrows(classOf[IndexOutOfBoundsException], a.get(3))
    assertThrows(classOf[IndexOutOfBoundsException], a.get(-1))
  }

  @Test def `Atomic Integer Array`(): Unit = {
    ...
    // the wrong indexes
    assertFalse(a.compareAndSet(0, 2, 3))
    assertFalse(a.compareAndSet(1, 0, 1))
    assertFalse(a.compareAndSet(2, 1, 2))
  ...
  @Test def `Atomic Integer Array is atomic`(): Unit = {
    val numThreads = 4
    testWithMinRepetitions() { n: Int =>
  ...
  @Test def `Atomic Long Array`(): Unit = {
    ...
    // the wrong indexes
    assertFalse(a.compareAndSet(0, 2L, 3L))
    assertFalse(a.compareAndSet(1, 0L, 1L))
    assertFalse(a.compareAndSet(2, 1L, 2L))
  ...
  @Test def `Atomic Long Array is atomic`(): Unit = {
    val numThreads = 4
    testWithMinRepetitions() { n: Int =>
