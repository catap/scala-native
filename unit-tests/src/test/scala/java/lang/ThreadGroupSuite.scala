...
class ThreadGroupSuite {
  @Test def `Constructors`(): Unit = {
    ...
    assertFalse(subgroup.parentOf(threadGroup))
  }

  @Test def `ThreadGroup.checkAccess does not crash`(): Unit = {
  ...
  @Test def `ThreadGroup.interrupt should interrupt sleep for all threads`()
      : Unit = {
    var fail = false
  ...
  @Test def `ThreadGroup.destroy`(): Unit = {
    val threadGroup = new ThreadGroup("abc")
    ...
    assertThrows(classOf[IllegalThreadStateException], threadGroup.destroy())
    assertFalse(threadGroup.isDestroyed)
    thread.goOn = false
    thread.join()
  ...
  @Test def `Thread.setPriority respects threadGroups maxPriority`(): Unit = {
  ...
  @Test def `A daemon thread group is automatically destroyed when its last thread is stopped or its last thread group is destroyed.`()
      : Unit = {
    ...
    threads.foreach { thread: Counter => thread.goOn = false }
    threads.foreach { thread: Counter => thread.join() }
  ...
  @Test def `activeCount, activeGroupCount, Thread.enumerate and list`()
      : Unit = {
    val structure = new Structure[Counter] {
      def makeThread(group: ThreadGroup, name: String) =
        new Counter(group, name)
      ...
      val thread = new Thread(group, "checker") {
        override def run() = {
          ...
          val correctValuesPartial = mutable.WrappedArray
            ...
            .forall { t => threads.contains(t) || t == this }
          ...
          val correctValuesLarge: scala.Boolean =
            threads.forall(prefix.contains) && prefix.contains(this)
  ...
  @Test def `ThreadGroup.suspend and resume should affect all threads`()
      : Unit = {
  ...
  @Test def `ThreadGroup.suspend and resume should respect allowThreadSuspension`()
      : Unit = {
    ...
    try {
      ...
      val countMap4: scala.collection.immutable.Map[Counter, scala.Long] =
        countMap4a ++ countMap4b
  ...
  @Test def `ThreadGroup.stop should stop all threads`(): Unit = {
